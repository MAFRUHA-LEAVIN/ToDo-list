...
class FirstPage extends React.Component {
  ...
  getWeather = (DEFAULT, API) => {
    ...
    fetch(
      `http://api.openweathermap.org/data/2.5/weather?q=${DEFAULT}&appid=${API}`
    ).then((response) => {
      const data = response.data;
      const bases = Object.keys(data.weather).map((bas) => {
        const obj = {
          base: bas,
          weather: data.weather[bas],
        };
        return obj;
      });
      console.log("ddd", bases);
      this.setState({ weather: bases });
    });
    ...
    return (
      <div>
        ...
        <div style={styled.rowlist1}>
          {this.state.weather.map((getw) => (
            <Weather key={getw.id} getw={getw} />
          ))}
        </div>
      </div>
    );
